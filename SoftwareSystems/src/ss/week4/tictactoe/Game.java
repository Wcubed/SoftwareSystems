...
public class Game {
	 ...
	 * Asks after each ended game if the user want to continue. Continues until the
	...
	/**
	 * Prints a question which can be answered by yes (true) or no (false). After
	 * prompting the question on standard out, this method reads a String from
	 * standard in and compares it to the parameters for yes and no. If the user
	 * inputs a different value, the prompt is repeated and the method reads input
	 * again.
	 * 
	 ...
	 * First the (still empty) board is shown. Then the game is played until it is
